...
void UpdateVehicleTimetable(Vehicle *v, bool travelling)
{
	...
	if (!real_current_order->IsType(OT_CONDITIONAL) &&
		...
		uint time_to_set;
		if (_settings_client.gui.timetable_mode == TimetableMode::Days) {
			time_to_set = CeilDiv(std::max(time_taken, 1), Ticks::DAY_TICKS) * Ticks::DAY_TICKS;
		} else {
			time_to_set = CeilDiv(std::max(time_taken, 1), Ticks::TICKS_PER_SECOND) * Ticks::TICKS_PER_SECOND;
		}
